...
def create_mock_player_data_bundle(
    player_id,
    gp,
    min_pg,
    pts_pg,
    ...
    basic_stats_data = {
        ...
        "min_per_game": min_pg,
        "pts_per_game": pts_pg,
        "fgm_per_game": pts_pg / 2,
        "fga_per_game": fga_pg,
        "fg_pct": fg_pct,
        "fg3m_per_game": fg3m_pg,
        "fg3a_per_game": fg3m_pg * 2 if fg3m_pg else 0,
        "fg3_pct": 0.35 if fg3m_pg else 0,
        "ftm_per_game": 2.0,
        "fta_per_game": fta_pg,
        "ft_pct": ft_pct,
        "oreb_per_game": reb_pg / 3,
        "dreb_per_game": reb_pg * 2 / 3,
        "reb_per_game": reb_pg,
        "ast_per_game": ast_pg,
        "tov_per_game": tov_pg,
        "stl_per_game": stl_pg,
    ...
    advanced_stats_data = {
        ...
        "tm_tov_pct": 0.12,
        "efg_pct": (
            (fg_pct + 0.5 * (fg3m_pg / fga_pg if fga_pg > 0 else 0))
            if fg_pct and fg3m_pg is not None
            else 0.5
        ),
    ...
    mock_season_stats = SeasonStats(
        ...
        player_age=25,
        basic_stats=mock_basic_stats,
        advanced_stats=mock_advanced_stats,
    )
...
MOCK_PLAYER_DATA_BUNDLES = {
    1: create_mock_player_data_bundle(
        player_id=1,
        gp=70,
        min_pg=30.0,
        pts_pg=25.0,
    ...
    2: create_mock_player_data_bundle(
        player_id=2,
        gp=65,
        min_pg=32.0,
        pts_pg=15.0,
    ...
    3: create_mock_player_data_bundle(
        player_id=3,
        gp=75,
        min_pg=20.0,
        pts_pg=8.0,
        ...
        ws=3.0,
        pie=0.08,
    ),  # Low volume for FG/FT PCT
    4: create_mock_player_data_bundle(
        player_id=4,
        gp=5,
        min_pg=25.0,
        pts_pg=20.0,
    ...
    5: create_mock_player_data_bundle(
        player_id=5,
        gp=70,
        min_pg=10.0,
        pts_pg=10.0,
    ...
    6: create_mock_player_data_bundle(
        player_id=6,
        gp=70,
        min_pg=30.0,
        pts_pg=18.0,
...
@patch(
    "app.main.nba_static_players.get_active_players",
    return_value=MOCK_ACTIVE_PLAYERS_LIST,
)
# Mock for app.main.get_player_data_from_nba_api
# This mock needs to return the correct bundle based on player_id
def mock_get_player_data_bundle(
    player_id, fetch_regular_season, fetch_playoffs, fetch_basic, fetch_advanced
):
...
class TestRecommendationEndpoint:
    ...
    def test_recommend_points_and_steals(
        self, mock_get_player_data, mock_get_active_players_static
    ):
        request_data = {"target_categories": ["PTS", "STL"], "num_recommendations": 2}
    ...
    def test_recommend_tov_lower_is_better(
        self, mock_get_player_data, mock_get_active_players_static
    ):
        request_data = {
    ...
    def test_recommend_fg_pct_volume_weighted(
        self, mock_get_player_data, mock_get_active_players_static
    ):
        request_data = {"target_categories": ["FG_PCT"], "num_recommendations": 2}
        response = client.post("/recommendations/categories", json=request_data)
        assert response.status_code == 200
        data = response.json()
        recommendations = data["recommendations"]
    ...
    def test_recommend_num_recommendations(
        self, mock_get_player_data, mock_get_active_players_static
    ):
    ...
    def test_recommend_filtered_players_not_included(
        self, mock_get_player_data, mock_get_active_players_static
    ):
    ...
    def test_recommend_unknown_category(
        ...
        request_data = {
            "target_categories": ["UNKNOWN_CAT", "PTS"],
            "num_recommendations": 1,
        }
        response = client.post("/recommendations/categories", json=request_data)
        assert (
            response.status_code == 200
        )  # Should still succeed, just ignore unknown category
        data = response.json()
        recommendations = data["recommendations"]
        assert len(recommendations) == 1
        ...
        assert (
            recommendations[0]["targeted_category_stats"]["UNKNOWN_CAT"] is None
        )  # Shows it was targeted but not scored

    def test_no_players_meet_criteria_or_available(
        self, mock_get_player_data, mock_no_active_players_fixture
    ):  # Renamed fixture
