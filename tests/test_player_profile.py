...
MOCK_NBA_PLAYERS_LIST = [
    ...
    {"id": 12345, "full_name": "Non Existent Player", "is_active": False}, # Test case for player not found by name
...
MOCK_DF_REG_SEASON_PG = pd.DataFrame({
    ...
    'GS': [54, 56],
    'MIN': [35.5, 37.2],
    'FGM': [11.1, 11.3], 'FGA': [22.2, 21.8], 'FG_PCT': [0.500, 0.520],
    'FG3M': [2.2, 2.9], 'FG3A': [6.9, 8.0], 'FG3_PCT': [0.321, 0.359],
    'FTM': [4.6, 4.8], 'FTA': [6.0, 6.5], 'FT_PCT': [0.768, 0.756],
    'OREB': [1.1, 1.1], 'DREB': [7.2, 6.5], 'REB': [8.3, 7.6], # Corrected REB calculation example
    'AST': [6.8, 6.2], 'STL': [0.9, 1.3], 'BLK': [0.6, 1.1], 'TOV': [3.2, 3.5], 'PF': [1.9, 2.2],
    'PTS': [28.9, 30.3]
})

# Mock DataFrame for PlayerProfileV2().career_totals_regular_season.get_data_frame() - PerGame
MOCK_DF_REG_CAREER_PG = pd.DataFrame({
    'PLAYER_ID': [2544], 'LEAGUE_ID': ['00'],
    'GP': [1421], 'GS': [1420],
    'MIN': [38.1], 'FGM': [10.0], 'FGA': [19.8], 'FG_PCT': [0.505],
    'FG3M': [1.6], 'FG3A': [4.5], 'FG3_PCT': [0.345],
    'FTM': [5.5], 'FTA': [7.5], 'FT_PCT': [0.735],
    'OREB': [1.2], 'DREB': [6.3], 'REB': [7.5],
    'AST': [7.3], 'STL': [1.5], 'BLK': [0.8], 'TOV': [3.5], 'PF': [1.9],
    'PTS': [27.2]
})
...
MOCK_DF_REG_SEASON_ADV = pd.DataFrame({
    ...
    'TEAM_ID': [1610612747, 1610612747],
    'TEAM_ABBREVIATION': ['LAL', 'LAL'],
    'PLAYER_AGE': [38.0, 37.0], 'GP': [55, 56], 'GS': [54, 56], 'MIN': [35.5, 37.2],
    'E_OFF_RATING': [113.0, 112.0], 'OFF_RATING': [115.7, 114.7],
    'E_DEF_RATING': [110.1, 109.1], 'DEF_RATING': [112.8, 111.8],
    'E_NET_RATING': [2.9, 2.8], 'NET_RATING': [2.9, 2.8],
    'AST_PCT': [0.320, 0.300], 'AST_TO': [2.13, 1.77], 'AST_RATIO': [19.4, 18.0],
    'OREB_PCT': [0.035, 0.036], 'DREB_PCT': [0.220, 0.210], 'REB_PCT': [0.128, 0.123],
    'TM_TOV_PCT': [12.0, 13.0], 'EFG_PCT': [0.550, 0.580], 'TS_PCT': [0.583, 0.619],
    'USG_PCT': [0.320, 0.315], 'E_USG_PCT': [0.325, 0.320],
    'E_PACE': [100.0, 99.5], 'PACE': [102.31, 101.5], 'PACE_PER40': [85.26, 84.58],
    'POSS': [4000, 4200], 'PIE': [0.180, 0.190],
    'FGM_PG': [11.1, 11.3], 'FGA_PG': [22.2, 21.8], # These are not usually in advanced stats but for testing parsing
...
MOCK_DF_REG_CAREER_ADV = pd.DataFrame({
    'PLAYER_ID': [2544], 'LEAGUE_ID': ['00'], 'TEAM_ID': [0],
    'GP': [1421], 'MIN': [38.1], 'OFF_RATING': [116.0], 'DEF_RATING': [106.1], 'NET_RATING': [9.9],
    'AST_PCT': [0.358], 'AST_TO': [1.98], 'AST_RATIO': [20.1],
    'OREB_PCT': [0.037], 'DREB_PCT': [0.197], 'REB_PCT': [0.117],
    'TM_TOV_PCT': [13.3], 'EFG_PCT': [0.545], 'TS_PCT': [0.588], 'USG_PCT': [0.315],
    'PACE': [96.0], 'PIE': [0.182], 'WS': [250.0]
})
...
class MockPlayerProfileV2:
    def __init__(self, player_id, per_mode36, season_type_all_star):
        ...
        if player_id == 999999: # Test Player With No Data
            self.season_totals_regular_season.get_data_frame = MagicMock(return_value=MOCK_DF_EMPTY)
            self.career_totals_regular_season.get_data_frame = MagicMock(return_value=MOCK_DF_EMPTY)
            self.season_totals_post_season.get_data_frame = MagicMock(return_value=MOCK_DF_EMPTY)
            self.career_totals_post_season.get_data_frame = MagicMock(return_value=MOCK_DF_EMPTY)
            return

        # Regular Season Data
        if self.season_type == "Regular Season":
            if self.per_mode36 == "PerGame":
                self.season_totals_regular_season.get_data_frame = MagicMock(return_value=MOCK_DF_REG_SEASON_PG.copy())
                self.career_totals_regular_season.get_data_frame = MagicMock(return_value=MOCK_DF_REG_CAREER_PG.copy())
            elif self.per_mode36 == "Advanced":
                self.season_totals_regular_season.get_data_frame = MagicMock(return_value=MOCK_DF_REG_SEASON_ADV.copy())
                self.career_totals_regular_season.get_data_frame = MagicMock(return_value=MOCK_DF_REG_CAREER_ADV.copy())
        ...
        # Playoff Data (simplified - returning empty for now, can be expanded)
            ...
            self.season_totals_regular_season.get_data_frame = MagicMock(return_value=MOCK_DF_EMPTY)
            self.career_totals_regular_season.get_data_frame = MagicMock(return_value=MOCK_DF_EMPTY)
            if self.per_mode36 == "PerGame": # Example: Mock playoff per game data if needed
                 # Use regular season mock data for playoffs for simplicity in this example
                self.season_totals_post_season.get_data_frame = MagicMock(return_value=MOCK_DF_REG_SEASON_PG.copy())
                self.career_totals_post_season.get_data_frame = MagicMock(return_value=MOCK_DF_REG_CAREER_PG.copy())
            elif self.per_mode36 == "Advanced":
                self.season_totals_post_season.get_data_frame = MagicMock(return_value=MOCK_DF_REG_SEASON_ADV.copy())
                self.career_totals_post_season.get_data_frame = MagicMock(return_value=MOCK_DF_REG_CAREER_ADV.copy())
            else:
                self.season_totals_post_season.get_data_frame = MagicMock(return_value=MOCK_DF_EMPTY)
                self.career_totals_post_season.get_data_frame = MagicMock(return_value=MOCK_DF_EMPTY)
...
class TestPlayerProfileEndpoints:
    ...
    def test_get_player_profile_lebron_regular_basic_advanced(self, mock_get_players, mock_ppv2):
        ...
        assert len(data['historical_regular_seasons']) == 2 # Based on MOCK_DF_REG_SEASON_PG/ADV
        ...
        assert data['latest_season_summary']['true_shooting_pct'] == 0.583 # From 2022-23 advanced
        ...
        mock_ppv2.assert_any_call(player_id=2544, per_mode36='PerGame', season_type_all_star='Regular Season')
        mock_ppv2.assert_any_call(player_id=2544, per_mode36='Advanced', season_type_all_star='Regular Season')


    def test_get_player_profile_lebron_playoffs_all(self, mock_get_players, mock_ppv2):
        response = client.get("/player/lebron-james?season_type=playoffs&stats_mode=all")
        assert response.status_code == 200
        data = response.json()
        assert data['player_id'] == 2544
        ...
        mock_ppv2.assert_any_call(player_id=2544, per_mode36='PerGame', season_type_all_star='Playoffs')
        mock_ppv2.assert_any_call(player_id=2544, per_mode36='Advanced', season_type_all_star='Playoffs')
    ...
    def test_get_player_profile_player_with_no_data(self, mock_get_players, mock_ppv2):
        ...
        response = client.get("/player/test-player-with-no-data?season_type=all&stats_mode=all")
        assert response.status_code == 200 # Should still succeed, but data fields will be None or empty lists
        data = response.json()
    ...
    def test_compare_players_endpoint(self, mock_get_players, mock_ppv2):
        response = client.get("/compare?player1=lebron-james&player2=stephen-curry&season_type=regular&stats_mode=all")
        assert response.status_code == 200
        data = response.json()
        ...
        assert data["lebron-james"]["career_regular_season"]["basic_stats"]["pts_per_game"] is not None
    ...
    def test_lineup_endpoint_basic(self, mock_get_players, mock_ppv2):
        ...
        # Avg should be 28.9
        assert "avg_points_per_game" in data["aggregated_stats_from_latest_season_summary"]
        # Note: MockPlayerProfileV2 returns LeBron's stats for Curry for simplicity, so avg is just LeBron's stats
        assert data["aggregated_stats_from_latest_season_summary"]["avg_points_per_game"] == 28.9
...
def test_parse_basic_stats_helper():
    from app.main import _parse_api_row_to_basic_stats # Import here for standalone test
